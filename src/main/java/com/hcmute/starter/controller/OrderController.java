                    ...
                    if (order.getAddressOrder().getId() == address1.getId()) {
                        ...
                        if (order.getPaymentOrder().getPaymentName().equals("paypal"))
                        {
                            try {
                                ...
                                AddNotificationRequest notificationRequest = new AddNotificationRequest(user.getId(),"Đơn hàng của bạn đã được đặt thành công","order");
                                UserNotificationEntity notification = userNotificationMapping.modelToEntity(notificationRequest);
                                userNotificationService.create(notification,user.getId());
                            }
                            catch (Exception e)
                            {
